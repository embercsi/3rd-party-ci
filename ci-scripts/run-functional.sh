...
for i in {1..30}; do
    if grep 'Now serving on' /ember-ci/artifacts/ember-csi.logs; then
        ...
        date
        ~vagrant/csi-sanity/csi-sanity-${X_CSI_SPEC_VERSION} --test.v --csi.endpoint=127.0.0.1:50051 --test.timeout 0 --ginkgo.v --ginkgo.progress 2>&1 | tee /ember-ci/artifacts/ci-sanity.logs
        test_result=$?
        date
        break
    fi
